...
public class ProcessorKeyPress extends Processor {
	...
	public boolean handle(Event e) {
		...
		if (null == action)
			return true;
		if (action.matches("Derp")) {
			Scripted.INSTANCE.bindArgument("Positional", Positional.INSTANCE);
			Scripted.INSTANCE.loadScript("derp");
			Scripted.INSTANCE.executeScript(((String)e.getInfo().get("guid")));
			return true;
