...
public class Client extends PApplet {
	...
	public void setup() {
		...
		try {
			...
			Spawner.INSTANCE.addObject("PlayerSpawner", new Object[] { "Player", new Vector2D(SCREEN_WIDTH / 2f, SCREEN_HEIGHT / 2f), 10, 10, 0xFFFFFF, 100 });
	...
	public void draw() {
		...
		while (myTime < ConnectionManager.INSTANCE.getRealTimeline().getElapsedTime()) {
			...
			if (ConnectionManager.INSTANCE.getGameTimeline().getElapsedTime() % (20 * minTickSize) == 0 && socket != null) {
				try {
					ThreadManager.INSTANCE.writeToQueue("Send" + getServerSockName(), new PacketLevelRequest());
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
			...
			} else if (socket == null && ConnectionManager.INSTANCE.getGameTimeline().getElapsedTime() % (20 * minTickSize) == 0) {
